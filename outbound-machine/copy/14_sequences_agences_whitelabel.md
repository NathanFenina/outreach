# 14 — Séquence AGENCES (pub/créa/marketing) · angle White-label + GEO

> Cible : agences de pub / créa / marketing **francophones** (FR + Suisse romande), extraites des
> audiences Agence-Ads (13, 36, 39) + les agences mal étiquetées dans "IT/Tech" (id 3).
> ⚠️ **Ne PAS envoyer aux germanophones (Werbeagentur) ni anglophones/saoudiens** → batch séparé, copy EN/DE.
> Angle : peer-to-peer, on leur fait **gagner de l'argent** (marque blanche), le **GEO** est le cheval de Troie
> (personne n'est équipé). Variables : `{{firstName}}` + `{{companyName}}` (présents en base).
> Signature : Nathan Fenina / Agence Decupler. Cadence J0 / J+2 / J+3 / J+5. Esprit UltB : on offre, zéro risque.
...
1. vous gérez déjà le SEO/GEO en interne (top, je vous laisse),
2. ce n'est pas le moment (on en reparle plus tard ?),
3. ce n'est pas vous qui gérez les partenariats (vers qui me tourner ?).
un mot et j'arrête. la grille d'audit GEO pour {{companyName}} reste à vous si vous la voulez.
Nathan Fenina
Agence Decupler
...
## Avant de publier
- Filtrer **francophones uniquement** (FR + .fr/.ch romands ; exclure Werbeagentur/.de-CH et .sa/anglophones).
- 1 campagne Lemlist : `Decupler · Agences · White-label GEO (FR)`.
- Variables `{{firstName}}` + `{{companyName}}` déjà en base → aucune prépa.
- ⚠️ Claim à garder VRAI : le « cas agence · budget annuel » (mail 3) — à confirmer ou reformuler si pas exact.
